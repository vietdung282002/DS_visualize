...
with col1:
    fig, ax = plt.subplots()
    sc = ax.scatter(df["salary_min"], df["salary_max"], c=df["yoe_min"], cmap="viridis", s=100, alpha=0.75, marker='o')
...
with col2:
    fig, ax = plt.subplots()
    sc = ax.scatter(df["salary_min"], df["salary_max"], c=df["yoe_max"], cmap="viridis", s=100, alpha=0.75, marker='o')
    ax.set_xlabel("Min Salary (triệu VND)")
    ax.set_ylabel("Max Salary (triệu VND)")
    ax.set_title("Scatter Plot of Salary Ranges with Years of Experience")
